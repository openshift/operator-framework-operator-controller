...
// The whole suite is gated on [OCPFeatureGate:NewOLMDeploymentConfig] so it is
// skipped automatically when that OCP feature gate is not enabled.
var _ = Describe("[sig-olmv1][OCPFeatureGate:NewOLMDeploymentConfig][Skipped:Disconnected] OLMv1 DeploymentConfig support", Ordered, Serial, func() {
	var (
	...
	installAndVerify := func(
		...
		DeferCleanup(func() {
			...
			helpers.EnsureCleanupClusterExtension(context.Background(), ceName, installNamespace)
		})
	...
	It("should apply environment variables from deploymentConfig to operator deployment containers",
		Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLMDeploymentConfig][Skipped:Disconnected] OLMv1 DeploymentConfig support should apply environment variables from deploymentConfig to operator deployment containers"),
	...
	It("should apply resource requirements from deploymentConfig to operator deployment containers",
		Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLMDeploymentConfig][Skipped:Disconnected] OLMv1 DeploymentConfig support should apply resource requirements from deploymentConfig to operator deployment containers"),
		func(ctx SpecContext) {
	...
	It("should apply tolerations from deploymentConfig to operator deployment pods",
		Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLMDeploymentConfig][Skipped:Disconnected] OLMv1 DeploymentConfig support should apply tolerations from deploymentConfig to operator deployment pods"),
		func(ctx SpecContext) {
	...
	It("should apply node selector from deploymentConfig to operator deployment pods",
		Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLMDeploymentConfig][Skipped:Disconnected] OLMv1 DeploymentConfig support should apply node selector from deploymentConfig to operator deployment pods"),
		func(ctx SpecContext) {
	...
	installAndExpectBlocked := func(
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(progressing.Reason).To(Equal(olmv1.ReasonBlocked), "expected Reason=Blocked for terminal error")
	...
	It("should reach a terminal blocked state when deploymentConfig.env has an invalid type",
		Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLMDeploymentConfig][Skipped:Disconnected] OLMv1 DeploymentConfig support should reach a terminal blocked state when deploymentConfig.env has an invalid type"),
		func(ctx SpecContext) {
	...
	It("should reach a terminal blocked state when deploymentConfig contains an unknown field",
		Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLMDeploymentConfig][Skipped:Disconnected] OLMv1 DeploymentConfig support should reach a terminal blocked state when deploymentConfig contains an unknown field"),
		func(ctx SpecContext) {
	...
	It("should apply annotations from deploymentConfig to operator deployment and its pod template",
		Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLMDeploymentConfig][Skipped:Disconnected] OLMv1 DeploymentConfig support should apply annotations from deploymentConfig to operator deployment and its pod template"),
		func(ctx SpecContext) {
