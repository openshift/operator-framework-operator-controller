...
func runNegativePreflightTest(ctx context.Context, scenario int) {
	...
	sa := helpers.NewServiceAccount(saName)
	...
	crb := helpers.NewClusterRoleBinding(crbName, crName, saName)
	...
	ce := helpers.NewClusterExtensionObject("openshift-pipelines-operator-rh", "1.15.0", ceName, saName)
