...
import (
    ...
    controllerdata "github.com/openshift/operator-framework-operator-controller/openshift/tests-extension/pkg/bindata/controller"
...
var _ = Describe("[sig-olmv1][OCPFeatureGate:NewOLM] OLMv1 operator installation",
    Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLM][Skipped:Disconnected] OLMv1 operator installation"), func() {
        var unique, nsName, ccName, rbName, opName, opCtrlName string
        BeforeEach(func() {
            ...
            opCtrlName = "install-test-op-ctrl" + unique
        })
        ...
        It("should block cluster upgrades if an incompatible operator is installed",
            Label("original-name:[sig-olmv1][OCPFeatureGate:NewOLM][Skipped:Disconnected] OLMv1 operator installation should block cluster upgrades if an incompatible operator is installed"), func(ctx SpecContext) {
                ...
                By("creating the controller BuildConfig")
                opCtrlCleanup := createBuildConfig(opCtrlName, nsName)
                DeferCleanup(opCtrlCleanup)

                By("creating the controller ImageStream")
                opCtrlIsCleanup := createImageStream(opCtrlName, nsName)
                DeferCleanup(opCtrlIsCleanup)

                By("creating the controller tarball")
                fileCtrl, fileCtrlCleanup := createTempTarBall(replacements, controllerdata.AssetNames, controllerdata.Asset)
                DeferCleanup(fileCtrlCleanup)
                By(fmt.Sprintf("created controller tarball %q", fileCtrl))
                ...
                opArgs := []string{
                    ...
                    fmt.Sprintf(
                        ...
                        nsName, opCtrlName, opCtrlName, nsName,
                    ),
                    "-f",
                    fileCtrl,
                }
                buildCtrl := startBuild(opArgs...)

                By(fmt.Sprintf("waiting for the build %q to finish", buildCtrl.Name))
                waitForBuildToFinish(ctx, buildCtrl.Name, nsName)
