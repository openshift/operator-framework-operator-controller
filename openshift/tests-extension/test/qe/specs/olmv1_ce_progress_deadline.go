...
var _ = g.Describe("[sig-olmv1][Jira:OLM][OCPFeatureGate:NewOLMBoxCutterRuntime] clusterextension progress deadline", g.Label("NonHyperShiftHOST"), func() {
	...
	g.It("PolarionID:88332-[OTP]A ClusterExtension is being upgraded and a persistent error prevents it from being fully rolled out", func(ctx g.SpecContext) {
		...
		expectClusterExtensionCondition(ctx, ceName, olmv1.TypeProgressing, metav1.ConditionTrue, olmv1.ReasonSucceeded, "")
		...
		expectActiveRevisions(ctx, ceName, ceName+"-1", ceName+"-2")

		g.By("verifying the new revision is still rolling out")
		expectClusterObjectSetCondition(ctx, ceName+"-2", olmv1.TypeProgressing, metav1.ConditionTrue, olmv1.ReasonRollingOut)

		g.By("verifying the new revision reports probe failure")
		expectClusterObjectSetCondition(ctx, ceName+"-2", olmv1.ClusterObjectSetTypeAvailable, metav1.ConditionFalse, olmv1.ClusterObjectSetReasonProbeFailure)
	})
})
