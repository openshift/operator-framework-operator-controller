...
var _ = g.Describe("[sig-olmv1][Jira:OLM] clustercatalog", g.Label("NonHyperShiftHOST", "ClusterCatalog"), func() {
	...
	g.It("PolarionID:69202-[Skipped:Disconnected][Skipped:Proxy]Catalogd clustercatalog offer the operator content through http server off cluster", func() {
		exutil.SkipOnProxyCluster(oc)
		var (
			baseDir                = exutil.FixturePath("testdata", "olm")
			clustercatalogTemplate = filepath.Join(baseDir, "clustercatalog.yaml")
	...
	g.It("PolarionID:74948-[Skipped:Disconnected]catalog offer the operator content through https server", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionTemplate     = filepath.Join(baseDir, "clusterextension.yaml")
	...
	g.It("PolarionID:75516-[Skipped:Disconnected][Skipped:SNO]CRD upgrade checks for the field maximum minimum changes", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			caseID                       = "75516"
			labelValue                   = caseID
