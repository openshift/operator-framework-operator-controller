...
func pullImage(ctx context.Context, imageRef, tmpDir string) error {
	...
	// On systems like podman/buildah, the default policy.json is in ~/.config/containers or /etc/containers.
	// In local/dev/test environments, we allow overriding the policy path via the POLICY_PATH environment variable.
	if policyPath := os.Getenv("POLICY_PATH"); policyPath != "" {
		absPath, err := filepath.Abs(policyPath)
		if err != nil {
			return fmt.Errorf("resolve policy path: %w", err)
		}
		if _, err := os.Stat(absPath); err != nil {
			return fmt.Errorf("policy file not found at %s: %w", absPath, err)
		}
		sysCtx.SignaturePolicyPath = absPath
	}
