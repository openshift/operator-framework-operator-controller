...
func pullImage(ctx context.Context, imageRef, tmpDir string) error {
	...
	// On Linux systems with tools like podman, buildah, or skopeo, a default policy.json may
	// be available at ~/.config/containers or /etc/containers.
	// However, this is not the case on mac/docker envs.
	// To avoid unnecessary setup complexity, we explicitly skip policy validation here only for local dev.
	// Load default policy from standard locations (~/.config/containers/policy.json or /etc/containers/policy.json)
	// If LOCAL_ENV=true, use a local insecure policy file
	if os.Getenv("LOCAL_ENV") == "true" {
		policyPath := filepath.Join("..", "..", "hack", "policy.json")
