...
// ExtractConfigsFromOCI extracts the /configs directory from image layers in OCI layout.
// Inside the /configs we have the catalog.json which represent the packages distributed
// in the indexes
func ExtractConfigsFromOCI(ociDir string) (string, error) {
	ctx := context.Background()
	...
	outDir := filepath.Join(ociDir, "configs")
	...
	for _, layer := range mf.LayerInfos() {
		...
		if err := extractConfigsFromTarball(rc, outDir); err != nil {
			rc.Close()
...
// extractConfigsFromTarball extracts files under /configs from a compressed layer tarball.
func extractConfigsFromTarball(r io.Reader, dest string) error {
	...
	for {
		...
		relPath := strings.TrimPrefix(hdr.Name, "configs/")
