...
func validateCatalogUnpack(t *testing.T) {
	...
	require.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		t.Logf("Checking arguments to assert.Equal: ct=%v", ct)
		t.Logf("Checking arguments to assert.Equal: metav1.ConditionTrue=%v", metav1.ConditionTrue)
		t.Logf("Checking arguments to assert.Equal: cond=%v", cond)
		t.Logf("Checking arguments to assert.Equal: cond.Status=%v", cond.Status)
