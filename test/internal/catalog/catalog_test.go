...
func TestBuildBundle_WithCRDAndDeployment(t *testing.T) {
	b := buildBundle("abc123", "test-abc123", "1.0.0", []BundleOption{WithCRD(), WithDeployment(), WithConfigMap()})
...
func TestBuildBundle_BadImage(t *testing.T) {
	b := buildBundle("bad1", "test-bad1", "1.0.2", []BundleOption{BadImage()})
...
func TestBuildBundle_NoCRD(t *testing.T) {
	b := buildBundle("nocrd", "test-nocrd", "1.0.0", []BundleOption{WithDeployment()})
	if _, ok := b.files["manifests/crd.yaml"]; ok {
